...
The `ssdlite_mobilenet_v2` model is used for object detection. For details, see a [paper](https://arxiv.org/pdf/1801.04381.pdf), MobileNetV2: Inverted Residuals and Linear Bottlenecks.
...
3. Detection box, name: `detection_boxes`. Contains detection boxes coordinates in format `[y_min, x_min, y_max, x_max]`, where (`x_min`, `y_min`)  are coordinates of the top left corner, (`x_max`, `y_max`) are coordinatesof the right bottom corner. Coordinates are rescaled to input image size.
