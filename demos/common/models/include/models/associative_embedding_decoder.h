...
struct Peak {
    Peak(const cv::Point2f& pos = cv::Point2f(-1, -1),
...
void findPeaks(const std::vector<cv::Mat>& nmsHeatMaps,
               ...
               int nmsHeatMapid, const int maxNumPeople);

std::vector<Pose> matchByTag(std::vector<std::vector<Peak>>& allPeaks,
                             int maxNumPeople, float detectionThreshold,
                             float tagThreshold, int numJoints,
