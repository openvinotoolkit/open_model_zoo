...
std::shared_ptr<InternalModelData> HpeAssociativeEmbedding::preprocess(const InputData& inputData, InferRequest::Ptr& request) {
    ...
    resizedImageSize = cv::Size(resizedImage.cols, resizedImage.rows);
    cv::Mat paddedImage;
...
std::unique_ptr<ResultBase> HpeAssociativeEmbedding::postprocess(InferenceResult & infResult) {
    ...
    *static_cast<ResultBase*>(result) = static_cast<ResultBase&>(infResult);
    ...
    float scaleX = internalData.inputImgWidth / static_cast<float>(resizedImageSize.width);
    float scaleY = internalData.inputImgHeight / static_cast<float>(resizedImageSize.height);

    for (auto& pose : poses) {
        for (auto& keypoint : pose.keypoints) {
            if (keypoint != cv::Point2f(-1, -1)) {
                keypoint.x *= outputScale;
                keypoint.x *= scaleX;

                keypoint.y *= outputScale;
                keypoint.y *= scaleY;
...
std::vector<HumanPose> HpeAssociativeEmbedding::extractPoses(
    ...
    std::vector<Pose> allPoses = matchByTag(allPeaks, maxNumPeople, detectionThreshold,
                                            tagThreshold, numJoints, useDetectionVal,
                                            ignoreTooMuch);
    // Filter poses
    auto it = std::begin(allPoses);
    while (it != std::end(allPoses)) {
        Pose pose = *it;
        float meanScore = pose.getMeanScore();
        if (meanScore < 0.1) {
    ...
    for (size_t i = 0; i < allPoses.size(); i++) {
        ...
        for (size_t j = 0; j < numJoints; j++) {
            ...
            keypoints.push_back(peak.pos);
        }
        HumanPose pose{keypoints, allPoses[i].getMeanScore()};
