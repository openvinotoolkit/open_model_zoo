...
int main(int argc, char *argv[]) {
    try {
        ...
        // --------------------------- Prepare input blobs -----------------------------------------------------
        mode->prepareInputBlobs();

        // --------------------------- Prepare output blobs -----------------------------------------------------
        mode->prepareOutputBlobs();
        // -----------------------------------------------------------------------------------------------------

        // --------------------------- 4. Loading model to the device ------------------------------------------
        slog::info << "Loading model to the device" << slog::endl;
        ExecutableNetwork userSpecifiedExecNetwork = ie.LoadNetwork(mode->getCnnNetwork(), FLAGS_d, userSpecifiedConfig);
        ExecutableNetwork minLatencyExecNetwork = ie.LoadNetwork(mode->getCnnNetwork(), FLAGS_d, minLatencyConfig);
        // -----------------------------------------------------------------------------------------------------
        ...
        if (!(mode->getImageInfoInputName()).empty()) {

           
            for (InferRequest::Ptr& requestPtr : userSpecifiedInferRequests) {//убрала const у requestPtr
                mode->setConstInput(requestPtr);
            }
            mode->setConstInput(minLatencyInferRequest);
        }
        ...
        while (true) {
            ...
            if (!requestResult.outputs.empty()) {
                ...
                /////////////////////
                mode->processOutput(requestResult.outputs, requestResult.frame,width, height, labels, FLAGS_r, FLAGS_t);
                //////////
            ...
            frameToBlob(frame, request, mode->getInputs().begin()->first);// возможно нужно определять input с информацией об изображении
            ...
            request->SetCompletionCallback([&mutex,
                                            ...
                                            &condVar] {
                {
                    ...
                    std::map<const std::string, Blob::Ptr> tmpOutputs;
                    for (auto it = mode->getOutputs().begin(); it != mode->getOutputs().end(); it++)
                    {
                        tmpOutputs.insert(std::pair<const std::string, Blob::Ptr>(it->first, std::make_shared<TBlob<float>>(*as<TBlob<float>>(request->GetBlob(it->first)))));
                    }
                    try {
