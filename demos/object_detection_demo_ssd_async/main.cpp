...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
    ...
    if (FLAGS_model_type.empty()) {
        throw std::logic_error("Parameter -model_type is not set");
    }
...
int main(int argc, char *argv[]) {
    try {
        ...
        /** Read labels (if any)**/
        model->loadLables(FLAGS_labels);
        // -----------------------------------------------------------------------------------------------------

        // --------------------------- 3. Configure input & output ---------------------------------------------
        // --------------------------- Prepare input blobs -----------------------------------------------------
        model->prepareInputBlobs(FLAGS_auto_resize);

        // --------------------------- Prepare output blobs -----------------------------------------------------
        model->prepareOutputBlobs();
        // -----------------------------------------------------------------------------------------------------

        // --------------------------- 4. Loading model to the device ------------------------------------------
        slog::info << "Loading model to the device" << slog::endl;
        ExecutableNetwork userSpecifiedExecNetwork = ie.LoadNetwork(model->getCnnNetwork(), FLAGS_d, userSpecifiedConfig);
        ExecutableNetwork minLatencyExecNetwork = ie.LoadNetwork(model->getCnnNetwork(), FLAGS_d, minLatencyConfig);
        // -----------------------------------------------------------------------------------------------------
        ...
        while (true) {
            ...
            frameToBlob(frame, request, model->getImageInputName()); 
            std::vector< std::string> tmpOutputsNames;
            for(size_t i=0;i< model->getOutputsNames().size();i++)
            tmpOutputsNames.push_back(model->getOutputsNames()[i]);
            ...
            request->SetCompletionCallback([&mutex,
                                            ...
                                            &condVar] {
                    {
                        ...
                        try {
                            std::map<std::string, Blob::Ptr> tmpOutputs;
                            for (size_t i = 0; i < tmpOutputsNames.size(); i++)
                            {
                                tmpOutputs.insert(std::pair< std::string, Blob::Ptr>(tmpOutputsNames[i], std::make_shared<TBlob<float>>(*as<TBlob<float>>(request->GetBlob(tmpOutputsNames[i])))));
                            }
