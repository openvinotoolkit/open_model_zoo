...
The inference backend is specified by -backend flag. Depends on openCV version there might be different backends available: `IE` starting from openCV 4.0, `ONNX` from 4.7.1 and `OV` from 4.8. The possible device value for `"ONNX/DML/<device>` can be found using `set OPENCV_LOG_LEVEL=DEBUG` and running the demo with `-backend ONNX/DML/`. Search for `Available DirectML adapters:`. For `-backend ONNX/OVEP/<device>` the device is the same as the one selected for `--use_openvino` while compiling `onnxruntime`.
