...
namespace hand_pose_estimation {
	...
	const size_t handPoseEstimator::keypointsNumber = 21;
...
std::vector<handPose> handPoseEstimator::estimate(const cv::Mat& image) {
    ...
    std::vector<handPose> poses = postprocess(
            heatMapsBlob->buffer(),
...
void handPoseEstimator::preprocess(const cv::Mat& image, float* buffer) const {
    cv::Mat resizedImage;
...
std::vector<handPose> handPoseEstimator::postprocess(
    ...
    std::vector<handPose> poses = extractPoses(heatMaps, pafs);
...
std::vector<handPose> handPoseEstimator::extractPoses(
        const std::vector<cv::Mat>& heatMaps,
    ...
    std::vector<handPose> poses = groupPeaksToPoses(
...
void handPoseEstimator::resizeFeatureMaps(std::vector<cv::Mat>& featureMaps) const {
...
void handPoseEstimator::correctCoordinates(std::vector<handPose>& poses,
...
bool handPoseEstimator::inputWidthIsChanged(const cv::Size& imageSize) {
