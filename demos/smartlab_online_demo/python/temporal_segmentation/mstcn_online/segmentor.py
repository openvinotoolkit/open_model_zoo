...
class Segmentor(object):
    ...
    def initialize(self, opt):
        ...
        his_buffer_len = 2 ** 11
        input_0 = torch.zeros((1, 2 * opt.embed_dim, his_buffer_len), dtype=torch.float).cuda()  # 1x2048x2048
    ...
    def action_segmentation(self, opt):
        ...
        # elif num_batch == 0:
        #     with torch.no_grad():
        #         embedding_unit1 = embedding_buffer1[:, start_index:end_index]
        #         embedding_unit2 = embedding_buffer2[:, start_index:end_index]
        #         feature_unit = np.concatenate([embedding_unit1[:, ], embedding_unit2[:, ]], axis=0)
        #         input = torch.tensor(feature_unit, dtype=torch.float).cuda()  # 2048xN
        ...
        #         """
        #         temporal_logits = predictions[:, :, :self.opt.num_classes, :]  # 4x1x16xN
        #         temporal_logits = F.softmax(temporal_logits[-1], 1)  # 1x16xN
        #         temporal_logits = temporal_logits.permute(0, 2, 1).squeeze(dim=0)
        #         temporal_logits = temporal_logits.detach().cpu().numpy()  # Nx16
        #     self.temporal_logits = np.concatenate([self.temporal_logits, temporal_logits], axis=0)
        #     if self.opt.sliding_smoothing:
        #         for embed_idx in range(start_index, end_index):
        ...
        #             self.temporal_predictions[time_start_idx:time_end_idx + 1] = self.temporal_logits[embed_idx]  # frame-based predictions
