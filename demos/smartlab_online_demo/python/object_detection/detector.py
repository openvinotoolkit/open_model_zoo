...
class Detector(object):
    def __init__(self, fp_top_models:list, fp_front_models:list, is_show:bool):
        ...
        ###
        self.top_model_path = fp_top_models
        self.front_model_path = fp_front_models
        ...
        self.classes = ["balance", "weights", "tweezers", "box",
            "battery", "tray", "ruler" , "rider" , "scale", "hand"]
    ...
    def initialize(self):
        ...
        ie = IECore()
        net = ie.read_network(
            model=self.top_model_path[0][:-4]+".xml",
            weights=self.top_model_path[0][:-4]+".bin"
        )
        self.top1_subdetector = ie.load_network(network=net, device_name="CPU")
        net = ie.read_network(
            model=self.top_model_path[1][:-4]+".xml",
            weights=self.top_model_path[1][:-4]+".bin"
        )
        self.top2_subdetector = ie.load_network(network=net, device_name="CPU")
        net = ie.read_network(
            model=self.front_model_path[0][:-4]+".xml",
            weights=self.front_model_path[0][:-4]+".bin"
        )
        self.front1_subdetector = ie.load_network(network=net, device_name="CPU")
        net = ie.read_network(
            model=self.front_model_path[1][:-4]+".xml",
            weights=self.front_model_path[1][:-4]+".bin"
        )
...
#     return bboxes, cls, scores
    ...
    def pseudo_detect(self, origin_img, input_blob, out_blob, exec_net, h, w, idx_offset:int):
        ...
        if dets is not None:
            ...
            # annotations = pseudolabel(final_boxes, final_cls_inds, final_scores, img_id, idx_offset, 0.3)
        else:
            return idx_offset, []
    ...
    def _detect_one(self, img, view='top'):
        ...
        input_blob = next(iter(sub_detector1.input_info))
        out_blob = next(iter(sub_detector1.outputs.keys()))
        _, _, h, w = sub_detector1.input_info[input_blob].input_data.shape
        # print(h, w)

        bboxes, cls, scores = self.pseudo_detect(
            img,
            input_blob,
            out_blob,
            sub_detector1,
