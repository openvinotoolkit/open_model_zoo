...
void read_wav(const std::string& file_name, RiffWaveHeader& wave_header, std::vector<int16_t>& wave) {
    ...
    inp_wave.read(reinterpret_cast<char*>(&wave_header), sizeof(RiffWaveHeader));

    std::string error_msg = "";
#define CHECK_IF(cond)                        \
    if (cond) {                               \
        error_msg = error_msg + #cond + ", "; \
    }
...
#undef CHECK_IF
    ...
    inp_wave.read(reinterpret_cast<char*>(&(wave.front())), wave_size * sizeof(int16_t));
}
...
void write_wav(const std::string& file_name, const RiffWaveHeader& wave_header, const std::vector<int16_t>& wave) {
    ...
    out_wave.write(reinterpret_cast<char*>(const_cast<::RiffWaveHeader*>(&wave_header)), sizeof(RiffWaveHeader));
    out_wave.write(reinterpret_cast<char*>(const_cast<int16_t*>(&wave.front())), wave.size() * sizeof(int16_t));
}
}  // namespace
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
        ...
        if (outputs.end() ==
            std::find_if(outputs.begin(), outputs.end(), [&out_state_name](const ov::Output<ov::Node>& output) {
    ...
    slog::info << "State_param_num = " << state_size << " (" << std::setprecision(4)
               << state_size * sizeof(float) * 1e-6f << "Mb)" << slog::endl;
    ...
    if (freq_data != freq_model) {
        slog::err << "Wav file " << FLAGS_i << " sampling rate " << freq_data << " does not match model sampling rate "
                  << freq_model << slog::endl;
    ...
    for (size_t i = 0; i < inp_wave_s16.size(); ++i) {
        inp_wave_fp32[i] = static_cast<float>(inp_wave_s16[i]) * scale;
    }
    ...
    slog::info << "\tSample length: " << std::fixed << std::setprecision(1) << patch_size * iter / (freq_data * 1e-3)
               << " ms" << slog::endl;
