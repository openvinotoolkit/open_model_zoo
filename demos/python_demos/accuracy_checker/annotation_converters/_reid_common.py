...
from accuracy_checker.representation import ReIdentificationAnnotation
...
def read_directory(directory, query, image_pattern):
    ...
    for image in directory.glob("*.jpg"):
        ...
        identifier = Path(directory.name) / image.name
...
def check_dirs(dirs, parent_dir, arg_name='data_dir'):
    for directory in dirs:
        if not directory.is_dir():
            message_pattern = (
                "{directory} not found in {parent_dir}. Check, "
                "that {arg_name} is pointed to a correct directory"
            )
            raise FileNotFoundError(message_pattern.format(
                directory=directory,
