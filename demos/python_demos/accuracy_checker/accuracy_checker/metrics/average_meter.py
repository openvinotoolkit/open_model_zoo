...
class AverageMeter:
    ...
    def evaluate(self):
        if self.total_count is None:
            return 0.0
        result = np.divide(self.accumulator, self.total_count,
                           out=np.zeros_like(self.accumulator), where=self.total_count != 0)
        return result
