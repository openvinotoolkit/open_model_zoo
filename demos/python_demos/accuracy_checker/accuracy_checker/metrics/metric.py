...
class Metric(ClassProvider):
    ...
    def __init__(self, config, dataset, name=None, state=None):
        ...
        self.configure()
        message_unsupport_multi_source = 'metric {} does not support several {} sources'
        ...
        if self.annotation_source and not is_single_metric_source(self.annotation_source):
            raise ConfigError(message_unsupport_multi_source.format(self.name, 'annotation'))
        ...
        if self.prediction_source and not is_single_metric_source(self.prediction_source):
            raise ConfigError(message_unsupport_multi_source.format(self.name, 'prediction'))
    ...
    def update(self, annotation, prediction):
        ...
        Updates metric value corresponding given annotation and prediction
        """
        pass
    ...
    def _resolve_representation_containers(self, annotation, prediction):
        ...
        return (resolve(annotation, self.annotation_types, 'annotation'),
                resolve(prediction, self.prediction_types, 'prediction'))
