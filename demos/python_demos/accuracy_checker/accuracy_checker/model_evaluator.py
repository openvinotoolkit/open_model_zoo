...
class ModelEvaluator:
    ...
    def process_dataset(self, stored_predictions=None, progress_reporter=None):
        if stored_predictions and Path(stored_predictions).is_file() or isinstance(self.launcher, DummyLauncher):
            self._annotations, self._predictions = self._load(stored_predictions, progress_reporter)
        ...
        for batch_id, (batch_annotation, batch_input) in enumerate(self.dataset):
            ...
            batch_predictions = self.launcher.predict(batch_identifiers, batch_input)
        ...
        if stored_predictions:
            self._store_predictions(stored_predictions, predictions_to_store)
    ...
    def compute_metrics(self):
        for result_presenter, evaluated_metric in self.metric_executor.iterate_metrics(
            ...
            result_presenter.write_result(evaluated_metric)

    def _load(self, stored_predictions, progress_reporter):
    ...
    @staticmethod
    def _store_predictions(stored_predictions, predictions):
        with open(stored_predictions, "wb") as f:
