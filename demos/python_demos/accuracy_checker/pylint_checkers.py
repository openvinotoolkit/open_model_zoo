...
class BackslashChecker(BaseChecker):
    ...
    name = 'backslash'
    msgs = {
        'W9901': ('use of \\ for line continuation',
    ...
    def process_module(self, node):
        with node.stream() as stream:
            for (line_number, line) in enumerate(stream):
                if line.decode().rstrip().endswith('\\'):
...
class AbsoluteImportsChecker(BaseChecker):
    ...
    msgs = {
        ...
        'W9902': ('absolute import from same package',
...
class StringFormatChecker(BaseChecker):
    ...
    msgs = {
        ...
        'W9903': ('use of "%" for string formatting',
    ...
    def visit_binop(self, node):
        if node.op != '%':
            return
        left = node.left
        if not (isinstance(left, astroid.Const) and
...
class BadFunctionChecker(BaseChecker):
    ...
    msgs = {
        ...
        'W9904': ('using prohibited function',
                  'bad-function-call',
                  '')
    }
