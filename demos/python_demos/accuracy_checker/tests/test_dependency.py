...
"""
from accuracy_checker.dependency import ClassProvider, _get_opts


def test_get_opts__positional_and_kw():
    opts = {'o': ((1,), {'a': 1})}
    args, kwargs = _get_opts(opts['o'])

    assert args == (1,)
    assert kwargs == {'a': 1}


def test_get_opts__kw_only():
    opts = {'o': {'a': 1}}
    args, kwargs = _get_opts(opts['o'])
...
def test_get_opts__positional_only():
    opts = {'o': (1, 2, 3)}
    args, kwargs = _get_opts(opts['o'])
