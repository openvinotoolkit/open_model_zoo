...
import pytest
from accuracy_checker.config.config_validator import (ConfigError, ConfigValidator, DictField, ListField, NumberField,
...
class TestDictField:
    ...
    def test_validates_valuess(self):
        dict_field = DictField()
        dict_field.validate({'foo': 42, 1: 'bar'})
...
class TestPathField:
    ...
    def test_path_is_checked(self):
        with mock_filesystem(['foo/bar']) as prefix:
            ...
            file_field = PathField(check_exists=True, is_directory=False)
            with pytest.raises(ConfigError):
                file_field.validate(prefix_path / 'foo')
            file_field.validate(prefix_path / 'foo' / 'bar')

            dir_field = PathField(check_exists=True, is_directory=True)
