...
def build_argparser():
    ...
    args.add_argument("-m", "--model", help="Required. Path to an .xml file with a trained model",
                      required=True, type=str)
    args.add_argument("--input_names", help="Required. Names for inputs in networks. For example ['input_ids','attention_mask','token_type_ids']",
                      required=True, type=str)
    args.add_argument("--output_names", help="Required. Names for outputs in networks. For example ['output_s','output_e']",
                      required=True, type=str)
...
# get big text and return list of token id and stren-end position for each id in original texts
...
# return whole sentence as start-end positions by given sub-sentence answer.
def find_sentence_range(context,s,e):
    #find start of sentence
    for c_s in range(s,max(0, s-200),-1):
        if context[c_s] in "\n\.":
            c_s += 1
            break
...
def main():
    ...
    if set(input_names_model) != set(input_names) or set(output_names_model) != set(output_names):
        ...
        log.error("     Expected input->output names: {}->{}".format(input_names, output_names))
    ...
    while True:
        ...
        #init some constant that defines token lists sizes
        #maximum number of tokens processed by network
        ...
        # token num between two neighbours context windows
        ...
        while c_e > c_s:
            ...
            #get non answer score (not valid if model has been fine-tuned on squad1.x)
            ...
            # check that answers list does not have found answer yet
