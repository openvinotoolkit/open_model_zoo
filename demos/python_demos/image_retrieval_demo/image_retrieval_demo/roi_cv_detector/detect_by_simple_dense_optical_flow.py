...
def get_diff_as_mask(frame, prev_frame, total_v, blur_kernel_size=5, max_diff_to_be_same=10):
    ...
    work_time = datetime.datetime.now() - begin_work_time
    work_time_ms = int(1000*work_time.total_seconds())
    log.debug("get_diff_as_mask: work_time = {} ms".format(work_time_ms))
    return mask1
...
class RoiDetector:
    ...
    def _create_default_roi_detector_impl(desired_min_side):
        ...
        roi_detector_impl = RoiDetectorImpl(cell_params, increase_rect_params, N_median, min_motion_to_work)
    ...
    def handle_frame(self, frame):
        ...
        * or None if it cannot make detection with sufficient confidence.
        ...
        cur_frame, scale = self._prepare_frame_for_default_roi_detector(frame, self.desired_min_side)
...
def handle_video(video_wrapper, last_frame, args, ann_avg_rect=None):
    ...
    Returns two values
    * res_bbox -- bounding box of the estimated ROI area on the video (may be None if detection is failed)
    * should_continue -- boolean value if processing of this video should be continued
    ...
    roi_detector = RoiDetectorImpl(cell_params, increase_rect_params, N_median, args.min_motion_to_work)
    ...
    while True:
        ...
        res_bbox = roi_detector.handle_frame(frame, prev_frame, frame_number)
        res_bbox_confidence = roi_detector.get_res_bbox_confidence()
    ...
    num_summed_masks = roi_detector.get_num_summed_masks()
    ...
    res_bbox = roi_detector.get_res_bbox()
    res_bbox_confidence = roi_detector.get_res_bbox_confidence()
    if res_bbox_confidence <= 0: # at the moment it either 0 or 1
        res_bbox = None
    result_img_to_show = roi_detector.get_result_img_to_show()
