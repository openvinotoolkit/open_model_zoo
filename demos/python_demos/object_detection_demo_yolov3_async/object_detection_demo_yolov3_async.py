...
import cv2
from openvino.inference_engine import IENetwork, IECore, StatusCode
...
def add_completion_result(active_request_data):
        request, frame_id, frame_mode, frame, start_time, completed_request_results, mode = active_request_data
...
def main():
    ...
    while cap.isOpened() or completed_request_results or active_requests_data:
        ...
        if next_frame_id_to_show in completed_request_results:
            ...
            if not args.no_show:
                ...
                if key == 9: # Tab key
                    ...
                    for active_request in active_requests_data.values():
                        add_completion_result(active_request)
                    active_requests_data.clear()
            ...
            infer_request_id = exec_nets[mode.current].get_idle_request_id()
            if infer_request_id < 0:
                status = exec_nets[mode.current].wait(num_requests=1)
                if status != StatusCode.OK:
                    raise Exception("Wait for idle request failed")
                infer_request_id = exec_nets[mode.current].get_idle_request_id()
                if infer_request_id < 0:
                    raise Exception("Invalid request id")
                for request, frame_id, *_ in active_requests_data.values():
                    if request == exec_nets[mode.current].requests[infer_request_id]:
                        add_completion_result(active_requests_data.pop(frame_id))
                        break
            request = exec_nets[mode.current].requests[infer_request_id]
