...
def get_objects(output, ng_func, new_frame_height_width, source_height_width, prob_threshold, is_proportional):
    objects = list()
    output_info = {
        node.get_friendly_name(): 
        (list(node.inputs()[0].get_source_output().get_node().shape), node._get_attributes())
        for node in ng_func.get_ordered_ops() if node.get_friendly_name() in output
    }

    for layer_name, out_blob in output.items():
        output_shape, params = output_info[layer_name]
...
def main():
    ...
    while (cap.isOpened() \
        ...
        if next_frame_id_to_show in completed_request_results:
            ...
            objects = get_objects(output, ng_func, (input_height, input_width), frame.shape[:-1], args.prob_threshold,
