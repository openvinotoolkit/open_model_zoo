...
class Model:
    def __init__(
        ...
        task_type, conversion_to_onnx_args, compilable
    ):
        self.name = name
    ...
    def deserialize(cls, model, name, subdirectory):
        with deserialization_context('In model "{}"'.format(name)):
            ...
            compilable = model.get('compilable', False)
            if not isinstance(compilable, bool):
                raise DeserializationError('"compilable": expected a boolean, got {!r}'.format(compilable))
