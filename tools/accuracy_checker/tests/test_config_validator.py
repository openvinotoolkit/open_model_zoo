...
class TestConfigValidationAPI:
    ...
    def test_launcher_config_without_framework(self):
        ...
        config_errors = ModelEvaluator.validate_config({'launchers': [launcher_config], 'datasets': []})
        assert len(config_errors) == 2
        assert config_errors[0].message == 'framework is not provided'
        assert config_errors[0].entry == launcher_config
        assert config_errors[0].field_uri == 'launcher'
        assert config_errors[1].message == 'datasets section is not provided'
        assert not config_errors[1].entry
        assert config_errors[1].field_uri == 'model'
    ...
    def test_unregistered_adapter_config(self):
        ...
        config_errors = ModelEvaluator.validate_config({'launchers': [launcher_config], 'datasets': []})
        assert len(config_errors) == 2
        assert config_errors[0].message.startswith('Invalid value "not_classification" for adapter')
        assert config_errors[0].entry == 'not_classification'
        assert config_errors[0].field_uri == 'adapter'
        assert config_errors[1].message == 'datasets section is not provided'
        assert not config_errors[1].entry
        assert config_errors[1].field_uri == 'model'
    ...
    def test_dataset_config_without_metrics(self):
        ...
        assert not config_errors[1].entry
        assert config_errors[1].field_uri == 'metrics'
    ...
    def test_data_reader_without_data_source(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message == 'Invalid value "None" for data_source: data_source is not allowed to be None'
        assert not config_errors[1].entry
        assert config_errors[1].field_uri == 'data_source'
    ...
    def test_unregistered_data_reader(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message.startswith('Invalid value "unknown" for dataset.reader')
        assert config_errors[-1].entry == 'unknown'
        assert config_errors[-1].field_uri == 'dataset.reader'
    ...
    def test_data_source_does_not_exists(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[-1].message == 'Invalid value "data_dir" for data_source: path does not exist'
    ...
    def test_data_source_is_file(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message == 'Invalid value "data" for data_source: path is not a directory'
    ...
    def test_annotation_is_not_provided(self):
        ...
        assert config_errors[1].field_uri == 'dataset'
    ...
    def test_annotation_conversion_without_converter(self):
        ...
        assert config_errors[1].field_uri == 'annotation_conversion'
    ...
    def test_annotation_conversion_missed_parameter(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message == 'Invalid config for imagenet: missing required fields: annotation_file'
        assert config_errors[1].entry == conversion_parameters
        assert config_errors[1].field_uri == 'imagenet'
    ...
    def test_annotation_conversion_extra_parameter(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message == "imagenet specifies unknown options: ['something_extra']"
        assert config_errors[1].entry == conversion_parameters
        assert config_errors[1].field_uri == 'imagenet'
    ...
    def test_preprocessing_config_unknown_type(self):
        ...
        assert config_errors[1].field_uri == 'preprocessing'
    ...
    def test_preprocessing_config_extra_parameter(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message == "resize specifies unknown options: ['something_extra']"
        assert config_errors[1].entry == preprocessing_config[1]
        assert config_errors[1].field_uri == 'resize'
    ...
    def test_postprocessing_config_unknown_type(self):
        ...
        assert config_errors[1].field_uri == 'postprocessing'
    ...
    def test_postprocessing_config_extra_parameter(self):
        ...
        assert not config_errors[0].entry
        assert config_errors[0].field_uri == 'model'
        assert config_errors[1].message == "resize_prediction_boxes specifies unknown options: ['something_extra']"
        assert config_errors[1].entry == postprocessing_config[0]
        assert config_errors[1].field_uri == 'resize_prediction_boxes'
