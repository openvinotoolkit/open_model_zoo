...
class SalienceMapMAE(PerImageEvaluationMetric):
    ...
    def update(self, annotation, prediction):
        foreground_pixels = prediction.mask[annotation.mask]
        foreground_error = np.size(foreground_pixels) - np.sum(foreground_pixels)
        background_error = np.sum(prediction.mask[~annotation.mask])
...
class SalienceMapFMeasure(PerImageEvaluationMetric):
    ...
    def update(self, annotation, prediction):
        ...
        label_and = np.logical_or(label3, annotation.mask)
        ...
        if num_and == 0:
            ...
            fmeassure = (1.3 * precision * recall) / (0.3 * precision + recall)
            self.recalls.append(recall)
            self.precisions.append(precision)
            self.fmeasure.append(fmeassure)
            return fmeassure
...
class SalienceEMeasure(PerImageEvaluationMetric):
    ...
    def update(self, annotation, prediction):
        ...
        score = np.sum(enhance_matrix)/(w * h - 1 + np.finfo(float).eps)
