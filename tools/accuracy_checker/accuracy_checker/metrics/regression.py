    ...
    def _calculate_diff_regression_rep(self, annotation, prediction):
        if isinstance(annotation.value, dict):
            ...
            for key in annotation.value:
                if not np.isscalar(annotation.value[key]) and np.issubdtype(annotation.value[key].dtype, np.integer):
                    annotation_val = annotation.value[key].astype(float)
                else:
                    annotation_val = annotation.value[key]
                if not np.isscalar(prediction.value[key]) and np.issubdtype(prediction.value[key].dtype, np.integer):
        ...
        if isinstance(prediction.value, dict):
            ...
            if not np.isscalar(annotation.value) and np.issubdtype(annotation.value.dtype, np.integer):
                annotation_val = annotation.value.astype(float)
            else:
                annotation_val = annotation.value
            prediction_val = next(iter(prediction.value.values()))
            if not np.isscalar(prediction.value) and np.issubdtype(prediction_val.dtype, np.integer):
        ...
        if not np.isscalar(annotation.value) and np.issubdtype(annotation.value.dtype, np.integer):
            ...
            annotation_val = annotation.value
        if not np.isscalar(prediction.value) and np.issubdtype(prediction.value.dtype, np.integer):
            prediction_val = prediction.value.astype(float)
        else:
