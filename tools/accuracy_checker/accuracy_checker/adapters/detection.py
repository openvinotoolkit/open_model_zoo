...
class UltralyticsEndToEndDetectionAdapter(Adapter):
    """
    Class for converting Ultralytics end-to-end detection output [N, 6] or [B, N, 6]
    with [x1, y1, x2, y2, score, label] to DetectionPrediction representation.
    """
    __provider__ = 'ultralytics_e2e_detection'
    prediction_types = (DetectionPrediction,)
    ...
    def parameters(cls):
        ...
        parameters.update({
            ...
            'score_threshold': NumberField(
                value_type=float, optional=True, default=0.0, min_value=0.0,
    ...
    def select_output_blob(self, outputs):
        ...
        if self.output_blob:
            ...
            return
        self.output_blob = self._find_output(outputs)
    ...
    def process(self, raw, identifiers, frame_meta):
        ...
        prediction_batch = predictions[self.output_blob]
        if np.ndim(prediction_batch) == 2 and len(identifiers) == 1:
            prediction_batch = np.expand_dims(prediction_batch, 0)

        result = []
        for identifier, pred, meta in zip(identifiers, prediction_batch, frame_meta):
            pred = np.asarray(pred)
            valid_mask = pred[:, 4] > self.score_threshold
            valid_detections = pred[valid_mask]

            if valid_detections.size == 0:
                result.append(DetectionPrediction(identifier, [], [], [], [], [], [], meta))
                continue

            x_mins = valid_detections[:, 0]
            y_mins = valid_detections[:, 1]
            x_maxs = valid_detections[:, 2]
            y_maxs = valid_detections[:, 3]
            scores = valid_detections[:, 4]
            labels = valid_detections[:, 5].astype(np.int32)

            result.append(DetectionPrediction(identifier, labels, scores, x_mins, y_mins, x_maxs, y_maxs, meta))
    ...
    def _find_output(predictions):
        filter_outputs = [
            output_name for output_name, out_data in predictions.items()
            if len(np.shape(out_data)) in [2, 3] and np.shape(out_data)[-1] == 6
        ]
        if not filter_outputs:
            raise ConfigError('Suitable Ultralytics end-to-end output layer not found')
        if len(filter_outputs) > 1:
            warnings.warn(
                'There are several suitable outputs {}. The first will be used. '.format(', '.join(filter_outputs)) +
                'If you need another layer, please specify output_blob explicitly'
            )
        return filter_outputs[0]
