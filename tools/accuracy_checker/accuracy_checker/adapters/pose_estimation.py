...
class HandPoseAdapter(Adapter):
    __provider__ = 'hand_pose_estimation'
    ...
    def process(self, raw, identifiers=None, frame_meta=None):
        ...
        for identifier, heatmap, paf, meta in raw_output:
            ...
            heatmap_avg = np.zeros((height, width, 22), dtype=np.float32)
            paf_avg = np.zeros((height, width, 22), dtype=np.float32)
            ...
            peak_counter = 0
            all_peaks = []
            for part in range(0, 22):  # 23th for bg
    ...
    @staticmethod
    def _add_pose_single_candidate(subset, candidate, idx_joint, kpt_num=23):
    ...
    @staticmethod
    def _add_pose_both_candidates(subset, temp, index_a, index_b, candidates, kpt_num=23):
    ...
    def group_peaks(self, peaks, pafs, kpt_num=23, threshold=0.05):
        subset = []
        ...
        for keypoint_id, maped_keypoints in enumerate(self.map_idx):
            score_mid = pafs[:, :, [x - 0 for x in maped_keypoints]]
    ...
    def get_poses(subset, candidate):
        ...
        for subset_element in subset:
            if subset_element.size == 0:
                continue
            keypoints_x, keypoints_y, keypoints_v = [0] * 21, [0] * 21, [0] * 21
