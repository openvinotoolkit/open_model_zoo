...
class ONNXIELauncherConfigValidator(LauncherConfigValidator):
    def __init__(
    ...
    def _set_model_source(self, framework):
        # self.need_conversion = framework.name != 'onnx_ie'
    ...
    def check_model_source(entry):
        ...
        specified = [FrameworkParameters(name='onnx', meta=False)]
        if not specified:
...
class ONNXIELauncher(Launcher):
    """
    Class for infer model using DLSDK framework.
    """

    __provider__ = 'onnx_ie'

    @classmethod
    def parameters(cls):
        parameters = super().parameters()
        parameters.update({
            'model': PathField(description="Path to model.", optional=True, file_or_directory=True),
    ...
    def __init__(self, config_entry, model_name='', delayed_model_loading=False):
        ...
        dlsdk_launcher_config = ONNXIELauncherConfigValidator(
            'ONNXIE_Launcher', fields=self.parameters(), delayed_model_loading=delayed_model_loading,
        )
        ...
        if not delayed_model_loading:
            ...
            # self._model  = ONNXIELauncher.get_model_path(self.config, dlsdk_launcher_config.framework)
    ...
    @staticmethod
    def get_model_path(config, framework=None):
        if framework is None:
            framework = ONNXIELauncherConfigValidator.check_model_source(config)
        ...
        #
        # mo_search_paths = []
        # model_optimizer = get_parameter_value_from_config(config, ONNXIELauncher.parameters(), '_model_optimizer')
        # if model_optimizer:
        ...
        model_name = Path(config_model).name.rsplit('.', 1)[0] #(
        #     Path(config_model).name.rsplit('.', 1)[0] or
        #     Path(config_weights).name.rsplit('.', 1)[0] or
        #     Path(config_meta).name.rsplit('.', 1)[0]
        # )

        # should_log_mo_cmd = get_parameter_value_from_config(config, ONNXIELauncher.parameters(), 'should_log_cmd')
        ...
        #     mo_search_paths,
        #     get_parameter_value_from_config(config, ONNXIELauncher.parameters(), 'mo_params'),
        #     get_parameter_value_from_config(config, ONNXIELauncher.parameters(), 'mo_flags'),
        #     get_parameter_value_from_config(config, ONNXIELauncher.parameters(), '_tf_custom_op_config_dir'),
        #     get_parameter_value_from_config(
        #         config, ONNXIELauncher.parameters(), '_tf_obj_detection_api_pipeline_config_path'
        #     ),
        #     get_parameter_value_from_config(
        #         config, ONNXIELauncher.parameters(), '_transformations_config_dir'
        #     ),
    ...
    def _device_specific_configuration(self):
        ...
        if 'CPU' in self._devices_list():
            if cpu_extensions:
                ...
                cpu_extensions = ONNXIELauncher.get_cpu_extension(cpu_extensions, selection_mode)
    ...
    def _create_network(self, input_shapes=None):
        ...
        if compiled_model:
            ...
            return
        self.network = self.read_network(self._model)
        # self.network = self.read_network('/nfs/inn/proj/vdp/vdp_tests/models/internal/onnx/pytorch/resnet34-ssd1200/resnet34-ssd1200.onnx')
    ...
    def load_network(self, network=None, log=False):
        ...
        if self.network:
            ...
            # self.exec_network = self.ie_core.load_network('/nfs/inn/proj/vdp/vdp_tests/models/internal/onnx/pytorch/resnet34-ssd1200/resnet34-ssd1200.onnx', self._device, num_requests=self.num_requests)
    ...
    def read_network(self, model):
        if 'read_network' in ie.IECore.__dict__:
            ...
            network = ie.IENetwork(model=str(model))

        # if 'read_network' in ie.IECore.__dict__:
        #     network = self.ie_core.read_network('/nfs/inn/proj/vdp/vdp_tests/models/internal/onnx/pytorch/resnet34-ssd1200/resnet34-ssd1200.onnx')
        # else:
