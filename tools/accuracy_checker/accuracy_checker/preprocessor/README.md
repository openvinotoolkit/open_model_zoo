...
* `face_patch` - crops faces detected in previous stage model from input image with vertical and horizontal scaling. `candidate_info` in `DetectionAnnotation` or `DetectionPrediction` format must be present in `annotation_meta`.
