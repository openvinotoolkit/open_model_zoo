...
class PointAligner(Preprocessor):
    ...
    def transformation_from_points(points1, points2):
        points1 = np.array(points1.astype(np.float64))
        points2 = np.array(points2.astype(np.float64))
        ...
        u, _, vt = np.linalg.svd(points1.T.dot(points2))
        r = (u.dot(vt)).T

        return np.hstack((points_std_ratio * r, (c2.T - (points_std_ratio * r).dot(c1.T)).reshape(2, -1)))
