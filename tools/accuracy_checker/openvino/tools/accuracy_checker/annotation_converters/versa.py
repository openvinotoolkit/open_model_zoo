...
class VersaDatasetConverter(FileBasedAnnotationConverter):
    ...
    def convert(self, check_content=False, progress_callback=None, progress_interval=100, **kwargs):
        ...
        data_dir = get_path(self.data_dir, is_directory=True)
        known_dir = get_path(self.data_dir / self.known_folder, is_directory=True)
        malicious_dir = get_path(self.data_dir / self.malicious_folder, is_directory=True)

        for entry_id, (identifier, kind) in enumerate(annotation):
            identifier = identifier.replace(':', '_')
            kind = int(kind)
            sample_path = malicious_dir / identifier if kind == 1 else known_dir / identifier
            if check_content:
                if not check_file_existence(sample_path):
                    content_errors.append('{}: does not exist'.format(sample_path))
            annotations.append(ClassificationAnnotation(str(Path(sample_path).relative_to(self.data_dir)), [kind,]))
