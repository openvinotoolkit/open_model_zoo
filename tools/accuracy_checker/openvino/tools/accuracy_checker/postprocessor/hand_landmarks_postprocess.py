...
class HandLandmarksPostprocessor(Postprocessor):
    ...
    def process_image_with_metadata(self, annotation, prediction, image_metadata=None):
        def draw_hand(draw, coords, fill):
            line_vxs = [range(5), range(5,9), range(9,13), range(13,17), [0] + list(range(17,21)), [1, 5, 9, 13, 17]]
            for vxs in line_vxs:
                c = [coords[i] for i in vxs]
                draw.line(c, fill=fill)

        for annotation_, prediction_ in zip(annotation, prediction):
            from PIL import Image, ImageDraw
            with Image.open("/localdisk/mzuevx/datasets/Hands/final/" + annotation_.identifier) as im:
                draw = ImageDraw.Draw(im)
                draw.rectangle(annotation_.metadata['rect'], outline=(0,0,255))
                draw_hand(draw, list(zip(annotation_.x_values, annotation_.y_values)), fill=(255,0,0))
            ...
            draw_hand(draw, list(zip(prediction_.x_values, prediction_.y_values)), fill=(0, 255, 0))
            im.save("./" + annotation_.identifier)
