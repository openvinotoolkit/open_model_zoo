...
2. If model inputs layout is different for APIs (e.g. for Tensorflow\* converted IR v11 with NHWC input layout, no conversion to NCHW with API 2.0 by default) and it specified in `inputs` section of launcher config (or override them using command line parameter `--layout`).
