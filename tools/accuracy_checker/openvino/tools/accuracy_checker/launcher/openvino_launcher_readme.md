...
Both launchers supports execution models in OpenVINO™ IR v10 and OpenVINO™ IR v11 formats, but
in some cases for successful launching, you need adjust configuration previously used with API 1.0:
1. After conversion model to IR v11 align output names in `adapter` if they are specified.
2. If model inputs layout is different for APIs (e.g. for Tensorflow\* converted IR v11 with NHWC input layout, no conversion to NCHW with API 2.0 by default) and it specified explicitly in config, you need align current input layout and config specified.
